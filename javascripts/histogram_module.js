var histogram_module = (function(url_trabajo, verbose){
	...
	// Asigna a una variable global una instancia del módulo de tabla.
	...
	// Inicializa variables globales y componentes visuales que son necesarios para la creación de histogramas.
	...
	// Crea el histograma de score decil del sistema de nicho, genera la interacción de tooltips, genera la interacción con histogramas y genera un line chart cuando el proceso de validación esta activado. Además enlaza al módulo de tabla para generar tabla de información por decil.
	function createMultipleBarChart(json_decil, array_recall, idComponent, nameMap) {
		...
		// $("#tbl_hist").css("margin-top", $("#map").height());
		  ...
		  svg.call(tip);
		    ...
		    x0.domain(json_decil.map(function(d) { return d.decil; }));
		    // _VERBOSE ? console.log("x0: " + x0.rangeBand()) : _VERBOSE;
		    x1.domain(ageNames.map(function(item, i){return item.name;})).rangeRoundBands([0, x0.rangeBand()]);


		    
		    totalmax_avg = d3.max(json_decil.map( function(d, index) {
	    		if (value_decil_ready <= index){
	    				return d3.max( d.values, function(d){ return parseFloat(d.p)}); 
	    		}
    		}));

		    totalmin_avg = d3.min(json_decil.map( function(d, index) { 
		    	if (value_decil_ready <= index){
		    		return d3.min(d.values, function(d){ return parseFloat(d.p)}); 
		    	}
		    }));
		    ...
		    maxmin_avg = d3.max(json_decil.map( function(d, index) { 
		    	if (value_decil_ready <= index){
		    		return d3.max(d.values, function(d){ if(d.p < 0) return d.p}); 
		    	}
		    }));
			...
			_VERBOSE ? console.log("NEG_DECIL: " + NEG_DECIL) : _VERBOSE;
		    _VERBOSE ? console.log("maxmin_avg: " + maxmin_avg) : _VERBOSE;
		    _VERBOSE ? console.log("totalmax_avg: " + totalmax_avg) : _VERBOSE;
		    _VERBOSE ? console.log("totalmin_avg: " + totalmin_avg) : _VERBOSE;
		    _VERBOSE ? console.log("totalmin_nice: " + totalmin_nice) : _VERBOSE;
		    _VERBOSE ? console.log("totalmax_nice: " + totalmax_nice) : _VERBOSE;

		    _VERBOSE ? console.log("y(maxmin_avg): ", y(maxmin_avg)) : _VERBOSE;
		    _VERBOSE ? console.log("max: " + Math.max(0, maxmin_avg)) : _VERBOSE;
			_VERBOSE ? console.log("x axis: " + translate_axis) : _VERBOSE;
		    ...
		    var state = svg.selectAll(".state")
		      ...
		      .enter().append("g")
		        .attr("class", "state")
		        .attr("transform", function(d) { return "translate(" + x0(d.decil) + ",0)"; });
	...
	function createBarChartFecha(distinctPoints){
		...
		// console.log("createBarChartFecha");
		...
		// console.log(colectas_dif);
		// console.log(colectas_zero);
		...
		var max_fecha = d3.max(colectas_dif.map(function(d) {return d.fechacolecta;}));
		var min_fecha = d3.min(colectas_dif.map(function(d) {return d.fechacolecta;}));
		...
		// console.log($("#hist_fecha_container").width());
		// console.log($("#hist_fecha_container").height());
		...
		x.domain(data.map(function(d) { return d.fechas; }));
        ...
        svg.selectAll(".bar")
          ...
          .enter().append("rect")
            .attr("class", "bar")
            .attr("x", function(d) { return x(d.fechas); })
	...
	// Crea los histogramas de épsilon especie, score especie y score celda del sistema de nicho, genera los tooltips y genera la interacción de los histogramas.
	function createBarChart(idComponent, data, f_legend){
		  ...
		  x.domain(data.map(function(d) { return d.bcenter; }));

		  max_freq = d3.max(data.map(function(d) {return parseFloat(d.frequency);}));
		  ...
		  svg.selectAll(".bar")
		    ...
		    .enter().append("rect")
		      .attr("class", "bar")
		      .attr("x", function(d) { return x(d.bcenter); })
		      .attr("width", x.rangeBand())
	...
	// Retorna un objeto de tipo BarChart para el histograma del sistema de comunidad.
	...
	// Es una clase que genera instancias de tipo histograma así como interacción con el módulo de tabla y el modulo de red a través del controlador res_diplay_net.
	function BarChart(json, display_obj) {
        ...
        function chart(div) {
            ...
            // _VERBOSE ? console.log("chart div method") : _VERBOSE;
            ...
            $.each(data, function( index, value ) {
                ...
                tvalue = value.value;
                // _VERBOSE ? console.log("tvalue: " + tvalue) : _VERBOSE;

                if(tvalue != 0)
                    value.value = tvalue / all.reduceCount().value();
                // _VERBOSE ? console.log(all.reduceCount().value() ) : _VERBOSE
                // _VERBOSE ? console.log(all.reduceSum(function(fact) { return fact.value; }).value() ) : _VERBOSE
            ...
            x.domain(data.map(function(d) { 
                // _VERBOSE ? console.log( parseFloat( (epsRange.invertExtent(d.key)[0] + epsRange.invertExtent(d.key)[1])/2).toFixed(2)) : _VERBOSE;
                avg = parseFloat( (display_obj.epsRange.invertExtent(d.key)[0] + display_obj.epsRange.invertExtent(d.key)[1])/2).toFixed(2);
                // _VERBOSE ? console.log(avg) : _VERBOSE
                return avg; 
            }));
                ...
                if (g.empty()) {
                    ...
                    g.selectAll(".bar")
                        ...
                        .attr("fill", function(d){
                            ...
                            // _VERBOSE ? console.log("left: " + epsRange.invertExtent(d.key)[0]) : _VERBOSE
                            // _VERBOSE ? console.log("right: " + epsRange.invertExtent(d.key)[1]) : _VERBOSE
                            // _VERBOSE ? console.log("min_val: " + min_val) : _VERBOSE
        ...
        brush.on("brush.chart", function(e) {
            ...
            // _VERBOSE ? console.log(localBrushStart) : _VERBOSE;
            // _VERBOSE ? console.log(localBrushEnd) : _VERBOSE;
        ...
        brush.on("brushend.chart", function() {
            ...
            if(brush.empty()){
                ...
                // _VERBOSE ? console.log(   d3.round(localBrushStart,0) ) : _VERBOSE;
                // _VERBOSE ? console.log(   d3.round(localBrushEnd,0) ) : _VERBOSE;
    ...
    // Realiza el ordenamiento de un json por un atributo especificado.
	...
	// Llama a la función que inicializa las variables necesarias para la creación de histogramas.
