var variable_module = (function(verbose, url_zacatuche){
	...
	var _TEXT_TOPO = 'topidx',
		_TEXT_ELEVACION = 'elevacion',
		_TEXT_PENDIENTE = 'pendiente';

	var _varfilter_selected,
		_time_selected,
		_id_text_input;

	var _value_vartree,
	 	_field_vartree,
	 	_parent_field_vartree,
	 	_level_vartree;

	var _arrayVarSelected = [],
		_groupvar_dataset = [],
		_var_sel_array = [],
		_arrayBioclimSelected = [],
	 	_groupbioclimvar_dataset = [],
	 	_arrayOtherSelected = [],
	 	_groupothervar_dataset = [];
	...
	function setTextInputId(id_text_input){
		_id_text_input = id_text_input;
	}
	...
	// Realiza la creación del selector de variables e inicializa los atributos necesarios para su correcto funcionamiento. Actualmente genera un selector de grupos taxonómicos, variables climáticas y variables topográficas.
	function VariableSelector(parent, id, title){
		...
		self.getChangeTreeVar = function (e, data){
			...
			if ($('#jstree_variables_species_' + id).jstree(true).get_top_selected().length > 0){
			    ...
			    for(i=0; i<headers_selected; i++){
			        ...
			        if(node_temp.attr.nivel == 2) level = _iTrans.prop('a_item_reino');
			          else if(node_temp.attr.nivel == 3) level = _iTrans.prop('a_item_phylum');
			            else if(node_temp.attr.nivel == 4) level = _iTrans.prop('a_item_clase');
			              else if(node_temp.attr.nivel == 5) level = _iTrans.prop('a_item_orden');
			                else if(node_temp.attr.nivel == 6) level = _iTrans.prop('a_item_familia');
			                  else if(node_temp.attr.nivel == 7) level = _iTrans.prop('a_item_genero');
			                    else if(node_temp.attr.nivel == 8) level = _iTrans.prop('a_item_especie');
			        ...
			        if(parent_node){
			            ...
			            if(node_temp.attr.nivel == 8){
			            	...
			            	self.arrayVarSelected.push({label: parent_node.text + " " + node_temp.text, level: level, numlevel: node_temp.attr.nivel, type: node_temp.attr.type, parent: parent_node.text });
			            }
			            else{
		...
		self.getChangeTreeVarBioclim = function (e, data){
		    ...
		    if ($('#jstree_variables_bioclim_' + id).jstree(true).get_top_selected().length > 0){
		        ...
		        for(i=0; i<headers_selected; i++){
		            ...
		            // _VERBOSE ? console.log(node_temp.id) : _VERBOSE;
		...
		self.existsGroup = function (arraySelected){
			...
			$.each(self.var_sel_array, function(index, item_sel){
	            ...
	            if(item_sel.value.length == arg_labels.length){
					...
					var value_labels = item_sel.value.map(function(val){ return val.label.split(" >> ")[1]; });
		...
		self.addOtherGroup = function (idTree, arraySelected, gpoName, idDivContainer, typeVar){
		    ...
		    if(self.groupDatasetTotal.length > 0)
		      	maxGroup = d3.max(self.groupDatasetTotal.map(function(d) {return d.groupid;}));
		...
		self.isSubset = function (set1, set2){
			return set1.every(function(val) { return set2.indexOf(val) >= 0; } );
		}
	...
	// Función que llama a la creación del selector de variables.
	...
	// el metodos genera la funcionalidad de los componentes y realiza distinción cuando existe mas de uno.
	...
	// Llama a la función que inicializa las variables necesarias para la creación del generador del selector de variables.
